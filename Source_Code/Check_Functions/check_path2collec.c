...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/25 21:48:49 by ysbai-jo          #+#    #+#             */
/*   Updated: 2024/03/25 21:48:50 by ysbai-jo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++x < engine->map.row)
	{
		...
		while (++y < engine->map.col)
		{
			if (engine->map.map[x][y] == WALL)
				...
				engine->map.vmap[x][y] = true;
			else if (engine->map.map[x][y] == EXIT)
				engine->map.vmap[x][y] = true;
			else
...
static void	compare_maps(t_game *engine)
{
	...
	while (++x < engine->map.row)
	{
		...
		while (++y < engine->map.col)
		{
			if (engine->map.map[x][y] == COLLEC && !engine->map.vmap[x][y])
				collec++;
		}
	}
	if (collec)
		err_path2collec(engine, collec, "you will need a plane to reach the");
	free_vmap(engine);
}
